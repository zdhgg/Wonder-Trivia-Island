...
e2e/support/knowledge-island.js # 知识岛选择器与文案（阶段名从应用纯函数取，不写第二份）
e2e/specs/                    # 七组核心场景
```
...
5. `specs/adventure-collection-book.spec.js` — 探险收藏册：首页与闯关地图都能打开，
   ...
   第一块「我的知识岛」按最近日期展示印章、无印章时是儿童化空状态。
6. `specs/home-reward-loop.spec.js` — 首页奖励闭环优化：3/3 时「宝箱可以打开啦」
   把宝箱滚进视野（不自动领取）、领取后提示消失、一次性奖励可点开收藏册、
   成长区知识岛摘要整行可点、欢迎区不再重复主线建议、1024 双列 / 390 单列不回归。
7. `specs/knowledge-island.spec.js` — 知识岛成长（Phase 2C-A）：0 / 3 / 7 / 15 / 30 枚
   印章分别对应哪个阶段、岛上多了哪些元素、下一阶段提示是否正确、最高阶段不再说
   「再攒 X 枚」、首页摘要与收藏册永远同一个阶段、390 无横向溢出、1024 首页双列不变；
   最后一条做作用域审计：章节 A → B 切换后知识岛逐字段不变（阶段 / 印章数 / 进度 /
   最近领取），而本章航海收藏、本章成就与顶部章节作用域一起换章。
